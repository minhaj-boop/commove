// src/app/(dashboard)/_components/ColStaffDashboard.tsx
...
export default function ColStaffDashboard() {
    ...
    useEffect(() => {
        setLoading(true);
        getRequestedMovementsForRole('col_staff').then((data) => {
    ...
    const name = user?.name ? user.name.split(' ').pop() : 'Col Staff';
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-fade-in">
                <div>
                    <h1 className="text-2xl font-bold text-foreground">
                        <span role="img" aria-label="staff" className="mr-2">🧑‍💼</span>
                        Welcome, {rank}{name}
                    </h1>
